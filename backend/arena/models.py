...
class GameMeta(BaseModel):
    ...
    max_players: int = Field(default=200, description="Upper bound on the roster. There is no small gameplay cap: a match takes as many players as join, up to a large storage-safety ceiling (the whole live match serializes into one item). Late joins are allowed; the match need not be full to start (see `min_players`).")
...
class MatchInfo(BaseModel):
    ...
    max_players: int = Field(default=200, description="Roster ceiling for this match (a large storage-safety bound, not a small gameplay cap). Players may keep joining up to this; the match starts at `min_players` and accepts late joins.")
