...
META = GameMeta(
    ...
    config_schema={
        "type": "object",
        "properties": {
            "seed": {"type": "integer", "default": 1, "description": "Deterministic market seed."},
            "horizon": {"type": "integer", "default": 40, "minimum": 1, "maximum": 100000,
                        "description": "Steps each agent trades through. Bounded: the price path is precomputed and stored, so an unbounded horizon would bloat persisted state."},
            "start_cash": {"type": "number", "default": 10000, "minimum": 0, "maximum": 1e12,
                           "description": "Starting cash for every agent."},
            "start_price": {"type": "number", "default": 100, "minimum": 0.01, "maximum": 1e9,
                            "description": "Price at step 0."},
            "drift": {"type": "number", "default": 0.0005, "minimum": -1.0, "maximum": 1.0,
                      "description": "Per-step expected return of the geometric walk."},
            "volatility": {"type": "number", "default": 0.02, "minimum": 0.0, "maximum": 10.0,
                           "description": "Per-step return standard deviation."},
            "max_position": {"type": "integer", "default": 100, "minimum": 0, "maximum": 1000000,
                             "description": "Position bound; holdings stay within [-max_position, +max_position]."},
        },
    },
    action_schema={
        "oneOf": [
    ...
    observation_schema={
        "type": "object",
        "description": "Partial view: prices only up to your own step; the future is hidden. Execution model: a trade fills immediately at `price` with NO fees and NO slippage. A buy (delta > 0) needs the resulting cash to stay non-negative; a short (delta < 0) is bounded only by max_position (no separate margin), and raises cash. The market parameters (drift, volatility, seed) are deliberately hidden; infer them from price_history. Every agent trades the identical price path, so this is pure skill; you never see a competitor's book, only how many there are.",
        "properties": {
            "asset": {"type": "string", "description": "Ticker of the traded instrument."},
            "horizon": {"type": "integer", "description": "Total steps you trade through; you are done at step == horizon."},
            "max_position": {"type": "integer", "description": "Holdings stay within [-max_position, +max_position]."},
            "step": {"type": "integer", "description": "Your index into the price path (starts at 0, +1 per submission)."},
            "steps_remaining": {"type": "integer", "description": "horizon - step: submissions left before your horizon. 0 means you are done."},
            "price": {"type": "number", "description": "Current price at your step (the last element of price_history)."},
            "price_history": {"type": "array", "items": {"type": "number"},
                              "description": "Prices from step 0 up to and including your current step."},
            "you": {"type": "object", "description": "Your own book (no competitor's book is ever shown).", "properties": {
                "cash": {"type": "number", "description": "Uninvested cash."},
                "position": {"type": "integer", "description": "Net holdings (negative = short)."},
                "equity": {"type": "number", "description": "cash + position * price, marked at your current step."},
                "done": {"type": "boolean", "description": "True once you reach the horizon; no more trades accepted."}}},
            "players": {"type": "integer", "description": "Number of competitors (a count only; their books are hidden)."},
        },
    },
)
...
class TradingDesk:
    ...
    def observe(self, state: State, player_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "step": t.step,
            "steps_remaining": max(0, state.cfg["horizon"] - t.step),
